...
export default class Appstore extends Component {
  state = {presstate: tabsList[0].tabId, search: ''}

  check = present => {
    this.setState(p => ({presstate: present}))
    console.log(present)
  }

  filtered = () => {
    const {presstate} = this.state
    console.log(typeof presstate)
    k = appsList.filter(each => each.category === presstate)
    return k
  }

  inp = event => {
    const k1 = event.target.value
    this.setState(prev => ({search: k1}))
  }
  ...
  secf = km => {
    ...
    const fres = km.filter(each => each.appName.toLowerCase().includes(search))
    return fres
  }

  render() {
    const {presstate, search} = this.state
    let km = this.filtered()
    km = this.secf(km)
    return (
      <div className="main">
        <h1>App Store</h1>
        <div className="search">
          <input type="search" placeholder="search" onChange={this.inp} />
          <img
        ...
        <div>
          <ul className="Appitem">
            {km.map(each => (
              <AppItem item={each} key={each.appId} prescat={presstate} />
            ))}
          </ul>
        </div>
      </div>
